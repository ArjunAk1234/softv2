...
import {
  ...
  Send, Star, CheckCircle, Loader2, XCircle, Clock
} from 'lucide-react';
...
const InterviewRoom = ({ session, role, token, candidateName, interviewerName, onLeave }) => {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      } catch { console.warn('Camera/mic unavailable'); }
      ...
      // ── Candidate: listen for interviewer ending the session ──
      ...
      pc.onicecandidate = (e) => {
        if (e.candidate) socket.emit('interview_event', { sessionId: session.id, type: 'ice', data: e.candidate });
      };

      socket.on('peer_joined', async ({ role: peerRole }) => {
        if (peerRole !== role && role === 'interviewer') {
  ...
  const handleLeave = () => { cleanup(); if (onLeave) onLeave(); };

  // Interviewer: submit score and END the session for candidate too
  ...
  // ── CANDIDATE: Interview ended by interviewer ─────────────────────────────
  if (endedByInterviewer) {
    return (
      <div className="fixed inset-0 bg-gray-950 z-50 flex items-center justify-center text-white">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          className="text-center max-w-md p-8">
          <div className="w-20 h-20 bg-orange-500/20 rounded-full flex items-center justify-center mx-auto mb-5">
            <XCircle className="w-10 h-10 text-orange-400" />
          </div>
          <h2 className="text-2xl font-bold mb-2">Interview Completed</h2>
          <p className="text-gray-400 mb-2">The interviewer has ended and scored this session.</p>
          <p className="text-sm text-gray-500 mb-8 flex items-center justify-center gap-1.5">
            <Clock className="w-4 h-4" /> Duration: {formatTime(elapsed)}
          </p>
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-6 text-sm text-yellow-300">
            ⏳ Your scores are being evaluated. The company will publish results shortly.
          </div>
          <button onClick={() => onLeave && onLeave()}
            className="px-8 py-3 bg-gray-700 hover:bg-gray-600 rounded-xl font-bold transition-colors">
            Back to Dashboard
          </button>
        </motion.div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-gray-950 z-50 flex flex-col text-white">
      ...
      <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
        <video ref={remoteVideoRef} autoPlay playsInline className="w-full h-full object-cover" style={{ display: connected ? 'block' : 'none' }} />
        {!connected && (
          ...
          <div className="absolute top-0 right-0 bottom-0 w-72 bg-gray-900/95 flex flex-col border-l border-gray-800">
            ...
            <div className="p-3 border-t border-gray-800 flex gap-2">
              <input value={msgInput} onChange={e => setMsgInput(e.target.value)}
                ...
                placeholder="Type a message…" className="flex-1 bg-gray-800 rounded-lg px-3 py-2 text-sm outline-none" />
              <button onClick={sendMessage} className="p-2 bg-blue-600 rounded-lg hover:bg-blue-500"><Send className="w-4 h-4" /></button>
            </div>
          </div>
        )}
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}
            ...
            {scoreDone ? (
              <div className="bg-gray-900 rounded-2xl p-8 text-center max-w-sm w-full">
                ...
                <p className="text-gray-400 mb-2">Interview score: <span className="text-green-400 font-bold">{score}/100</span></p>
                <p className="text-xs text-yellow-400 mb-6">The candidate has been notified that the session has ended.</p>
                <button onClick={handleLeave} className="w-full py-3 bg-gray-700 rounded-xl font-bold hover:bg-gray-600">
                  Leave Room
                </button>
              </div>
            ) : (
              <div className="bg-gray-900 rounded-2xl p-6 max-w-sm w-full space-y-4">
                ...
                <p className="text-xs text-orange-400 bg-orange-500/10 border border-orange-500/30 rounded-lg px-3 py-2">
                  ⚠️ Submitting will end the session for the candidate too.
                </p>
                <div>
      ...
      <div className="h-20 bg-gray-900 flex items-center justify-center gap-4 flex-shrink-0">
        <button onClick={toggleMic} className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${micOn ? 'bg-gray-700 hover:bg-gray-600' : 'bg-red-600 hover:bg-red-500'}`}>
          {micOn ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}
        </button>
        <button onClick={toggleCam} className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${camOn ? 'bg-gray-700 hover:bg-gray-600' : 'bg-red-600 hover:bg-red-500'}`}>
          {camOn ? <Video className="w-5 h-5" /> : <VideoOff className="w-5 h-5" />}
        </button>
        <button onClick={() => setShowChat(c => !c)} className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors relative ${showChat ? 'bg-blue-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
        ...
        {/* Candidate leave (doesn't end for interviewer) */}
        {role === 'candidate' && (
          <button onClick={handleLeave} className="w-14 h-12 rounded-full bg-red-600 hover:bg-red-500 flex items-center justify-center transition-colors">
            <PhoneOff className="w-5 h-5" />
          </button>
        )}
        {/* Interviewer leave only after scoring */}
        {role === 'interviewer' && (
          <button onClick={handleLeave} className="w-14 h-12 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center transition-colors" title="Leave (score first)">
            <PhoneOff className="w-5 h-5" />
          </button>
        )}
      </div>
