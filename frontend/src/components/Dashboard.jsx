...
import {
  Briefcase, ClipboardList, User, CheckCircle, Clock, TrendingUp,
  ArrowRight, Calendar, Users, BarChart2, Loader2, Plus, Trophy,
  FileText, AlertCircle, Building2, Video
} from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { Link } from 'react-router-dom';
import InterviewerDashboard from './InterviewerDashboard';
import InterviewRoom from './InterviewRoom';  // ← candidate joins from here
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-3xl font-bold text-text dark:text-dark-text">Welcome, {auth.user.name}!</h1>
            <p className="text-purple-600 dark:text-purple-400 mt-1">Manage your hiring pipeline with AI-powered tools</p>
          </div>
          <div className="ml-auto">
            <Link to="/jobs" className="flex items-center gap-2 px-5 py-2.5 bg-primary dark:bg-dark-primary text-background dark:text-dark-background rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity">
              <Plus className="w-4 h-4" /> Post a Job
            </Link>
          </div>
        </div>
...
const CandidateDashboard = ({ auth }) => {
  const [applications, setApplications] = useState([]);
  const [loading, setLoading] = useState(true);
  const [activeSession, setActiveSession] = useState(null); // ← interview session to join
  ...
  // ── FIX: fetch interview session for a specific application ──────────────
  const joinInterview = async (appId) => {
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.error || 'Interview session not found');
        return;
      }
      const session = await res.json();
      console.log('[CandidateDashboard] joining interview session:', session);

      if (!session.id) {
        alert('Interview session has no ID — contact support');
        return;
      }
      setActiveSession(session);
    } catch (e) {
      console.error('[CandidateDashboard] joinInterview error:', e);
      alert('Network error. Please try again.');
    }
  };

  // ── Render InterviewRoom when candidate joins ────────────────────────────
  if (activeSession) {
    console.log('[CandidateDashboard] mounting InterviewRoom, session.id =', activeSession.id);
    return (
      <InterviewRoom
        ...
        interviewerName={activeSession.interviewer_name}
  ...
  // Applications where interview is scheduled (show Join button)
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-3xl font-bold text-text dark:text-dark-text">Welcome back, {auth.user.name}!</h1>
            <p className="text-primary dark:text-dark-primary mt-1">Track your job applications and interview progress</p>
          </div>
          <div className="ml-auto">
            <Link to="/jobs" className="flex items-center gap-2 px-5 py-2.5 bg-primary dark:bg-dark-primary text-background dark:text-dark-background rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity">
              <Plus className="w-4 h-4" /> Browse Jobs
      ...
      {/* ── FIX: Interview Join Banner ──────────────────────────────────────── */}
      {interviewApps.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.15 }}
          className="bg-purple-50 dark:bg-purple-900/20 border-2 border-purple-300 dark:border-purple-700 rounded-2xl p-5"
        >
          <h2 className="text-base font-bold text-purple-800 dark:text-purple-300 mb-3 flex items-center gap-2">
            <Video className="w-5 h-5" /> Your Scheduled Interviews
          </h2>
          <div className="space-y-3">
            {interviewApps.map(app => {
              const isScheduled = app.status === 'interview_scheduled';
              return (
                <div key={app.id} className="flex items-center justify-between gap-4 p-4 bg-white/70 dark:bg-dark-background/50 rounded-xl border border-purple-200 dark:border-purple-800">
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-text dark:text-dark-text truncate">{app.job_title}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{app.company_name}</p>
                  </div>
                  {isScheduled ? (
                    <button
                      onClick={() => joinInterview(app.id)}
                      className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-bold hover:opacity-90 transition-all flex-shrink-0"
                    >
                      <Video className="w-4 h-4" /> Join Interview
                    </button>
                  ) : (
                    <span className="flex items-center gap-1 text-green-600 dark:text-green-400 text-sm font-semibold flex-shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Pipeline */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }} className={card}>
          <h2 className="text-xl font-bold text-text dark:text-dark-text mb-5">Pipeline</h2>
          <div className="space-y-3">
            {[
              { label: 'Applied', val: count('applied') + count('resume_reviewed') + count('shortlisted'), color: 'bg-blue-500' },
